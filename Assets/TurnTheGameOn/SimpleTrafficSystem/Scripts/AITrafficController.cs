...
{
    /*
    public class AITrafficController : Singleton<AITrafficController>
    {
        #region Variables
        ...
        private JobHandle carTransformpositionJobHandle;
        private JobHandle frAITrafficCarWheelJobHandle;
        private JobHandle flAITrafficCarWheelJobHandle;
        private JobHandle brAITrafficCarWheelJobHandle;
        private JobHandle blAITrafficCarWheelJobHandle;
            ...
            if (isInitialized)
            {
                ...
                for (int i = 0; i < carCount; i++) // operate on results
                {
                    if (isActiveArray[i])
                    {
                            ...
                            if (enableLaneChanging)
                            {
                                if (speedArray[i] > minSpeedToChangeLanes)
                                {
                                    ...
                                    if (frontHit[i] == true && canChangeLanes[i] && isChangingLanes[i] == false)
                                    {
                                        ...
                                        if (TryCast(rightOriginList[i], 
                                                    sideSensorSize, 
                                                    rightDirectionList[i], 
                                                    rightRotationList[i], 
                                                    sideSensorLength,
                                                    out var rightResults))
                                        {
                                            var boxHit = rightResults[0];
                        ...
                        if (isDrivingArray[i])
                        {
                            MoveWheelCollider(frontRightWheelColliderList, FRwheelPosition_CachedArray, FRwheelQuaternion_CachedArray, moveSteerArray[i], i);
                            MoveWheelCollider(frontLefttWheelColliderList, FLwheelPosition_CachedArray, FLwheelQuaternion_CachedArray, moveSteerArray[i], i);
                            MoveWheelCollider(backRighttWheelColliderList, BRwheelPosition_CachedArray, BRwheelQuaternion_CachedArray, 0, i);
                            MoveWheelCollider(backLeftWheelColliderList, BLwheelPosition_CachedArray, BLwheelQuaternion_CachedArray, 0, i);
                        }
                ...
                carTransformpositionJobHandle = carTransformpositionJob.Schedule(carTransformAccessArray);
                ...
                frAITrafficCarWheelJobHandle = frAITrafficCarWheelJob.Schedule(frontRightWheelTransformList);
                ...
                flAITrafficCarWheelJobHandle = flAITrafficCarWheelJob.Schedule(frontLeftWheelTransformList);
                ...
                brAITrafficCarWheelJobHandle = brAITrafficCarWheelJob.Schedule(backRightWheelTransformList);
                ...
                blAITrafficCarWheelJobHandle = blAITrafficCarWheelJob.Schedule(backLeftWheelTransformList);

                carTransformpositionJobHandle.Complete();
                frAITrafficCarWheelJobHandle.Complete();
                flAITrafficCarWheelJobHandle.Complete();
                brAITrafficCarWheelJobHandle.Complete();
                blAITrafficCarWheelJobHandle.Complete();
        ...
        public void Set_IsDrivingArray(int _index, bool _value)
        {
            if (isDrivingArray[_index] != _value)
            {
                ...
                if (_value == false)
                {
                    ...
                    MoveWheelCollider(frontRightWheelColliderList, FRwheelPosition_CachedArray, FRwheelQuaternion_CachedArray, moveSteerArray[_index], _index);
                    MoveWheelCollider(frontLefttWheelColliderList, FLwheelPosition_CachedArray, FLwheelQuaternion_CachedArray, moveSteerArray[_index], _index);
                    MoveWheelCollider(backRighttWheelColliderList, BRwheelPosition_CachedArray, BRwheelQuaternion_CachedArray, 0, _index);
                    MoveWheelCollider(backLeftWheelColliderList, BLwheelPosition_CachedArray, BLwheelQuaternion_CachedArray, 0, _index);
                }
            }
        }
            ...
            if (currentDensity < _AITrafficPool.density) //Spawn Traffic
            {
                ...
                for (int i = 0; i < currentAmountToSpawn; i++)
                {
                    ...
                    if (availableSpawnPoints[randomSpawnPointIndex].waypoint.onReachWaypointSettings.parentRoute.currentDensity < availableSpawnPoints[randomSpawnPointIndex].waypoint.onReachWaypointSettings.parentRoute.maxDensity)
                    {
                        ...
                        spawncar.transform.SetPositionAndRotation(spawnPosition,
        ...
        private bool TryCast(Vector3 center, Vector3 halfExtends, Vector3 direction, Quaternion orientation, float distance, out RaycastHit[] result)
        {
            const int capacity = 1;

            result = new RaycastHit[capacity];

            return Physics.BoxCastNonAlloc(center,
                                           halfExtends,
                                           direction,
                                           result,
                                           orientation,
                                           distance,
                                           layerMask, QueryTriggerInteraction.UseGlobal) > 0;
        }

        private void MoveWheelCollider(List<WheelCollider> wheelColliders, NativeArray<float3> wheelPositionCached, NativeArray<quaternion> wheelQuaternionCached, float steerAngle, int index) 
        {
            var currentWheelCollider = wheelColliders[index];

            currentWheelCollider.motorTorque = moveAccelArray[index];
            currentWheelCollider.brakeTorque = moveFootBrakeArray[index];

            currentWheelCollider.GetWorldPose(out var wheelPosition, out var wheelRotation);
            currentWheelCollider.steerAngle = steerAngle;

            wheelPositionCached[index] = wheelPosition;
            wheelQuaternionCached[index] = wheelRotation;
        }
    }*/
    ...
    public class AITrafficController : Singleton<AITrafficController>
    {
        ...
        public class Access 
        {
            private AITrafficController traffic;
            private int index;

            public Access(int index) 
            {
                this.traffic = AITrafficController.Instance;
                this.index = index;
            }
